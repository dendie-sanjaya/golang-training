...
func (handler *UserWalletHandler) Transfer(ctx context.Context, req *pb.TransferRequest) (*pb.TransferResponse, error) {
	...
	jsonData, err := json.Marshal(historyCreditResponse)
	if err != nil {
		log.Fatalf("Error marshalling to JSON: %v", err)
	}
	message := string(jsonData)

	send_kafka_topic_transaction(brokers, topic, message)
...
func send_kafka_topic_transaction(brokers []string, topic string, message string) error {
	...
	// Kirim pesan ke topik "transaction"
	partition, offset, err := producer.SendMessage(kafkaMessage)
	if err != nil {
		return fmt.Errorf("failed to send message to Kafka: %v", err)
	}

	log.Printf("Message sent to Kafka partition %d at offset %d\n", partition, offset)
	return nil
}
