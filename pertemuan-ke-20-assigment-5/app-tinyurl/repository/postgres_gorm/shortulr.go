...
func (r *shortUrlRepository) CreateShortUrl(p_urllong string) (entity.ShortUrl, error) {
	...
	if err := r.db.Table("short_urls").Create(&urlShort).Error; err != nil {
...
func (r *shortUrlRepository) GetShortUrl(p_shortUrl string) (entity.ShortUrl, error) {
	...
	if err := r.db.Table("short_urls").Select("url_long", "url_short").Where("url_short = ?", p_shortUrl).First(&urlShort).Error; err != nil {
